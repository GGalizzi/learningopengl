...
impl World {
    pub fn from_area(area: Area) -> World {
        ...
        for y in 0..area.height * expand {
            let uy =
                (y as f32 / expand as f32).floor() as usize;
            for z in 0..area.depth * expand {
                let uz = (z as f32 / expand as f32).floor()
                    as usize;
                for x in 0..area.width * expand {
                    let ux = (x as f32 / expand as f32)
                        .floor()
                        as usize;
                    let tile =
                        area.tiles[uy][uz * area.width + ux];

                    build(tile, x, y, z, &mut chunks);
                }
            }
        }
    ...
    pub fn from_area2(area: Area) -> World {
        let mut chunks = HashMap::new();
...
fn build(
    ...
    .as_();
    //println!("goes in chunk {:?}", chunk_pos);
    let chunk =
        chunks.entry(chunk_pos).or_insert(Chunk::empty());
